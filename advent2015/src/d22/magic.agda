...
calc-turn : Spell → Situation → Maybe Situation
calc-turn sp situation with (apply-eff-damage (apply-eff-timer situation))
calc-turn sp situation | pre-turn with (can-afford sp (p-mana pre-turn) ∧ (valid-effect sp (act-eff pre-turn)))
calc-turn sp situation | pre-turn | false = nothing
calc-turn sp situation | pre-turn | true with (apply-p-damage (apply-cost pre-turn sp) sp)
calc-turn sp situation | pre-turn | true | p-turn with (p-dead p-turn)
calc-turn sp situation | pre-turn | true | p-turn | true = just p-turn
calc-turn sp situation | pre-turn | true | p-turn | false with (apply-eff-damage (apply-eff-timer p-turn))
calc-turn sp situation | pre-turn | true | p-turn | false | eff-turn with (b-dead eff-turn)
calc-turn sp situation | pre-turn | true | p-turn | false | eff-turn | true = just eff-turn
calc-turn sp situation | pre-turn | true | p-turn | false | eff-turn | false = just (apply-b-damage eff-turn)

calc-turn-with-hist : List Spell → Spell → Situation → Maybe (List Spell × Situation)
calc-turn-with-hist hist sp sit1 with (calc-turn sp sit1)
calc-turn-with-hist hist sp sit1 | nothing = nothing
calc-turn-with-hist hist sp sit1 | (just sit2) = just ((concat (hist ∷ (sp ∷ []) ∷ [])) , sit2)

calc-spell-seq : List Spell → Situation → List String
calc-spell-seq [] sit1 = show-sit sit1 ∷ []
calc-spell-seq (sp ∷ xs) sit1 with (calc-turn sp sit1)
calc-spell-seq (sp ∷ xs) sit1 | nothing = (show-sit sit1 ++ " ==> " ++ (sp-name sp) ++ " ==> " ++ show-sitm nothing) ∷ []
calc-spell-seq (sp ∷ xs) sit1 | (just sit2) with (calc-spell-seq xs sit2)
calc-spell-seq (sp ∷ xs) sit1 | (just sit2) | tail = (show-sit sit1 ++ " ==> " ++ sp-name sp) ∷ tail

search-valid-h-layer : Nat → (List Spell × Situation) → (List (List Spell × Situation) × List (List Spell × Situation))
search-valid-h-layer  best (hist , sit1) = complete , inprogress
  where
    next-level : List (List Spell × Situation)
    next-level = unmaybe (map (λ {sp → calc-turn-with-hist hist sp sit1 }) all-spells)
    complete : List (List Spell × Situation)
    complete = filterᵇ (λ {(_ , q) →  b-dead q}) next-level
...
search-valid-h : Nat → Nat → List (List Spell × Situation) → List (List Spell × Situation)
search-valid-h 0 _ _ = []
search-valid-h _ _ [] = []
search-valid-h (suc l) best queue = concat (complete ∷ (search-valid-h l new-best inprogress) ∷ [])
  where
    next-layers : List (List (List Spell × Situation) × List (List Spell × Situation))
    next-layers = map (search-valid-h-layer best) queue
    complete : List (List Spell × Situation)
...
cheapest-spells : Situation → (Nat × List Spell)
cheapest-spells sit1 = min-score
  where
    valid : List (List Spell × Situation)
    valid = search-valid-h 1300 10000000 (([] , sit1) ∷ [])
...
cheapest-real-spells _ = show (proj₁ pair) ++ " <-- " ++ show-sp-l (proj₂ pair) ++ "\n"
  where
    pair : Nat × List Spell
    pair = cheapest-spells (sit (statb 10 10) start-hp start-mana boss-hp [])

test-calc-turn : show-sitm (calc-turn (spell "Magic Missile" 53 4 0 noeff) (sit (statb 10 10) start-hp start-mana 51 [])) ≡ "40/447 vs 47"
test-calc-turn = refl

test-calc-seq : unlines ( calc-spell-seq (map sp-by ("Poison" ∷ "Magic Missile" ∷ [])) (sit (statb 8 10) 10 250 13 [])) ≡
  "10/250 vs 13 with [] ==> Poison\n" ++
  "2/77 vs 10 with [poison:5] ==> Magic Missile\n" ++
  "2/24 vs 0 with [poison:3]"
test-calc-seq = refl

test-calc-seq-c : unlines ( calc-spell-seq (map sp-by ("Poison" ∷ "Recharge" ∷ "Magic Missile" ∷ "Poison" ∷ "Drain" ∷ "Magic Missile" ∷ [])) (sit (statb 9 10) 50 500 41 [])) ≡
...
test-calc-seq-b : (unlines ∘ calc-spell-seq (
  ...
  (sp-by "Drain") ∷
  (sp-by "Poison") ∷
  (sp-by "Magic Missile") ∷ [])) (sit (statb 8 10) 10 250 14 []) ≡
...
test-search-valid-h : length (search-valid-h 150 100000 (([] ,
  (sit (statb 8 10) 10 250 13 [])) ∷ [])) ≡ 1
test-search-valid-h = refl

test-cheapest-spells-ex : (show ∘ proj₁) (cheapest-spells (sit (statb 8 10) 10 250 13 [])) ≡ "226"
test-cheapest-spells-ex = refl

test-cheapest-spells-ex-b : (show ∘ proj₁) (cheapest-spells (sit (statb 8 10) 10 250 14 [])) ≡ "641"
